...
__global__ void kernel_shared_memory(double *x_pos, double *y_pos,
                                     ...
                                     double resolution) {
  size_t const idx = (size_t)blockDim.x * blockIdx.x + threadIdx.x;
  if (idx >= atoms_len)
    return;

  // Shared memory layout: x/y/z
  ...
  for (int block_id = blockIdx.x + 1; block_id < gridDim.x; block_id++) {
    ...
    size_t i = (size_t)blockDim.x * block_id + threadIdx.x;
    if (i < atoms_len) {
    ...
    size_t valid_count =
        min((size_t)blockDim.x, atoms_len - (size_t)block_id * blockDim.x);
